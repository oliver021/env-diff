...
`@oliver021/env-drift` is a zero-dependency CLI that keeps your `.env` files honest — it diffs any number of environment files at once, audits for leaked secrets, and integrates with GitHub Actions in a single `npx` command.
...
```bash
# Compare two files
npx @oliver021/env-drift compare .env .env.production

# Compare three files side by side (matrix view)
npx @oliver021/env-drift compare .env .env.staging .env.production

# Audit a file for leaked secrets and doc drift
npx @oliver021/env-drift audit .env

# Push local vars to Vercel (dry-run first)
VERCEL_TOKEN=xxx npx @oliver021/env-drift sync vercel .env --dry-run
```
...
npm install -D @oliver021/env-drift

# Or run on-demand with npx — no install needed
npx @oliver021/env-drift <command>
```
...
```bash
@oliver021/env-drift compare <file1> <file2> [file3 ...] [options]
```
...
```bash
@oliver021/env-drift audit [envFile] [options]
```
...
```bash
@oliver021/env-drift sync <provider> [envFile] [options]
```
...
VERCEL_TOKEN=xxx @oliver021/env-drift sync vercel .env --project my-app --dry-run

# Push for real (prompts for confirmation)
VERCEL_TOKEN=xxx @oliver021/env-drift sync vercel .env --project my-app

# Pull production vars for review
VERCEL_TOKEN=xxx @oliver021/env-drift sync vercel .env.production --pull
```
...
@oliver021/env-drift audit .env --baseline-create .envdiff-baseline.json

# Re-audit — existing findings suppressed, new ones surface
@oliver021/env-drift audit .env --baseline .envdiff-baseline.json --fail-on-audit
```
...
```yaml
- name: Run @oliver021/env-drift audit
  run: |
    npx -y @oliver021/env-drift audit .env.example \
      --output sarif > @oliver021/env-drift.sarif
  continue-on-error: true

- name: Upload SARIF
  uses: github/codeql-action/upload-sarif@v3
  with:
    sarif_file: @oliver021/env-drift.sarif
```
...
```yaml
repos:
  - repo: https://github.com/node-utils/@oliver021/env-drift
    rev: v1.0.0
    hooks:
      - id: @oliver021/env-drift-audit
```
...
@oliver021/env-drift audit .env --output json --no-gitignore | jq '.issues[] | select(.severity == "high")'

# Count SARIF results
@oliver021/env-drift audit .env --output sarif | jq '.runs[0].results | length'
```
...
Add `--verify-secrets` to have `@oliver021/env-drift` call each provider's API and confirm whether a detected key is **actually live**. A live key is upgraded to `critical` severity.

```bash
@oliver021/env-drift audit .env --verify-secrets --fail-on-audit --min-severity critical
```
...
`@oliver021/env-drift` auto-detects the format from the file extension. You can override it with `-f, --format`.
