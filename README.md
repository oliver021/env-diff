...
`env-diff` is a zero-dependency CLI that keeps your `.env` files honest — it diffs any number of environment files at once, audits for leaked secrets, and integrates with GitHub Actions in a single `npx` command.
...
```bash
env-diff compare <file1> <file2> [file3 ...] [options]
```
...
```bash
env-diff audit [envFile] [options]
```
...
```bash
env-diff sync <provider> [envFile] [options]
```
...
VERCEL_TOKEN=xxx env-diff sync vercel .env --project my-app --dry-run

# Push for real (prompts for confirmation)
VERCEL_TOKEN=xxx env-diff sync vercel .env --project my-app

# Pull production vars for review
VERCEL_TOKEN=xxx env-diff sync vercel .env.production --pull
```
...
env-diff audit .env --baseline-create .envdiff-baseline.json

# Re-audit — existing findings suppressed, new ones surface
env-diff audit .env --baseline .envdiff-baseline.json --fail-on-audit
```
...
```yaml
repos:
  - repo: https://github.com/node-utils/env-diff
    rev: v1.0.0
    hooks:
      - id: env-diff-audit
```
...
env-diff audit .env --output json --no-gitignore | jq '.issues[] | select(.severity == "high")'

# Count SARIF results
env-diff audit .env --output sarif | jq '.runs[0].results | length'
```
...
Add `--verify-secrets` to have `env-diff` call each provider's API and confirm whether a detected key is **actually live**. A live key is upgraded to `critical` severity.

```bash
env-diff audit .env --verify-secrets --fail-on-audit --min-severity critical
```
...
`env-diff` auto-detects the format from the file extension. You can override it with `-f, --format`.
